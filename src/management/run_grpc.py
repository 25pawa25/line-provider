...
from clients.grpc.proto.line_provider import line_provider_pb2_grpc
from clients.grpc.servicer.line_provider import get_line_provider_servicer
...
class Command(BaseCommand):
    ...
    async def start_server(self, host: str, port: int):
        ...
        server = grpc.aio.server(
            interceptors=(logging_interceptor, signature_interceptor)
        )
        line_provider_pb2_grpc.add_LineProviderServicer_to_server(servicer, server)
    ...
    async def servicer(self):
        return await get_line_provider_servicer()
